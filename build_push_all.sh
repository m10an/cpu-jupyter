...
export TAGNAME="v1.4_cpu_ubuntu-20.04"
...
docker build --no-cache -t cschranz/cpu-jupyter:$TAGNAME .build/
...
docker tag $IMG_ID cschranz/cpu-jupyter:$TAGNAME
docker rm -f cpu-jupyter_1
docker run -d -it -p 8848:8888 -v $(pwd)/data:/home/jovyan/work -e GRANT_SUDO=yes -e JUPYTER_ENABLE_LAB=yes --user root --restart always --name cpu-jupyter_1 cschranz/cpu-jupyter:$TAGNAME

docker push cschranz/cpu-jupyter:$TAGNAME
...
docker build -t cschranz/cpu-jupyter:${TAGNAME}_slim  .build/

export IMG_ID=$(docker image ls | grep ${TAGNAME}_slim  | head -1 | awk '{print $3}')
echo "push image with ID $IMG_ID and Tag ${TAGNAME}_slim."

docker tag $IMG_ID cschranz/cpu-jupyter:${TAGNAME}_slim
docker push cschranz/cpu-jupyter:${TAGNAME}_slim
...
docker build -t cschranz/cpu-jupyter:${TAGNAME}_python-only  .build/
...
docker tag $IMG_ID cschranz/cpu-jupyter:${TAGNAME}_python-only
docker push cschranz/cpu-jupyter:${TAGNAME}_python-only
